...
function calcMeasure(line, point) {
	var measure = 0.0;
	var sortedSegments = buildSortedSegmentCollection(line, point);
	var closestSegment = sortedSegments[0];
	var pointOnSegment = closestOnSegment(point, closestSegment.segment);
	sortedSegments.sort(function (a, b) {
		return a.index - b.index;
	});
	for (var i = 0; i < sortedSegments.length; i++) {
		var start = sortedSegments[i].segment[0];
		var end = sortedSegments[i].segment[1];
		if (closestSegment.index == i) {
			var dx = pointOnSegment[0] - start[0];
			var dy = pointOnSegment[1] - start[1];
	...
	return {'measure': measure,'pointOnSegment':pointOnSegment};
}

function buildSortedSegmentCollection(line, point) {
	var index = 0;
	var sortedSegments = [];
	line.forEachSegment(function (start, end) {
		var seg = {};
		var roundedStart = [start[0], start[1]];
		var roundeEnd = [end[0], end[1]];
		seg["segment"] = [roundedStart, roundeEnd];
		seg["index"] = index++;
		seg["distToPoint"] = squaredDistanceToSegment(point, [roundedStart, roundeEnd]);
...
function handleMeasure(e) {
	...
	if (e.selected.length == 1) {
		...
		var result = calcMeasure(feature.getGeometry(), e.mapBrowserEvent.coordinate);
		var point = new ol.geom.Point(result.pointOnSegment);
		point.transform(map.getView().getProjection(), ol.proj.get("EPSG:28992"));
		element.popover('destroy');
		element.popover({
			'placement': 'top',
			'html': true,
			'content': 'rd-new: ' + point.getCoordinates()[0].toFixed(2) + ', ' + point.getCoordinates()[1].toFixed(2) + '<br/>Measure: ' + result.measure.toFixed(2)
		});
		popup.setPosition(result.pointOnSegment);
...
function buildRailConnectionLayer(railConnections) {
	...
	map.on('pointermove', function (e) {
		if (measureFeature) {
			var result = calcMeasure(selectedRailConnection.getGeometry(), e.coordinate);
			measureFeature.getGeometry().setCoordinates(result.pointOnSegment);
			measureFeature.set('label', result.measure.toFixed(2));
		}
	});
	
	selectInteraction.getOverlay().setStyle(measureStyleFunction);
	selectInteraction.on('select', function (e) {
		...
		console.log(message);
		if (e.selected.length == 1) {
			selectedRailConnection = e.selected[0];
			var point = new ol.geom.Point(e.mapBrowserEvent.coordinate);
			measureFeature = new ol.Feature({
					geometry: point
				});
			selectInteraction.getOverlay().getSource().addFeature(measureFeature);
		} else {
			measureFeature = undefined;
			selectedRailConnection = undefined;
		}
		handleMeasure(e);
	});
}
